...
namespace Largest_product_in_a_grid__11_
{
    class Program
    {
...
01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48".Replace("\r\n"," ").Split(" ");
            ...
            for(var j = 0; j < 17; j++)
            {
                ...
                while (indexI >= j + 3)
                {
                    for (var i = 0; i < 4; i++)
                    {
                        cRes *= grid[indexI - i, indexJ + i];
                        map[indexI - i, indexJ + i] = $"{indexI - i}:{indexJ + i}";
                    }
                    if (cRes > res)
                    {
                        res = cRes;
            ...
            for(var i = 16; i>= 0; i--)
            {
                ...
                while(indexI < 17)
                {
                    for(var j = 0; j < 4; j++)
                    {
                        cRes *= grid[indexI + j, indexJ + j];
                        map[indexI + j, indexJ + j] = $"{indexI + j}:{indexJ + j}";
                    }
                    if (cRes > res)
                    {
                        res = cRes;
